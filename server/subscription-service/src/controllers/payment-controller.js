...
exports.createOrder = async (req, res) => {
  try {
    ...
    console.error('PayPal Order Creation Error:', e.response ? e.response.data : e.message);
    res.status(500).json({
      success: false,
      message: e.response ? e.response.data.message : "Error while creating paypal order",
    });
  }
};
