...
const buildEditor = ({
  ...
  return {
    ...
    addHeart: () => {
      ...
      const path = new fabric.Path('M 0 200 v -200 h 200 a 100 100 90 0 1 0 200 a 100 100 90 0 1 -200 0 z');
      path.set({
        ...DIAMOND_OPTIONS,
        fill: fillColor,
        stroke: strokeColor,
    ...
    addArrow: () => {
      ...
      // Create arrow shape with polygon
      const arrowWidth = WIDTH * 0.5;
      const arrowHead = HEIGHT * 0.4;
      const stemWidth = WIDTH * 0.15;

      const points = [
        { x: 0, y: (HEIGHT - stemWidth) / 2 },
        { x: WIDTH - arrowHead, y: (HEIGHT - stemWidth) / 2 },
        { x: WIDTH - arrowHead, y: 0 },
        { x: WIDTH, y: HEIGHT / 2 },
        { x: WIDTH - arrowHead, y: HEIGHT },
        { x: WIDTH - arrowHead, y: (HEIGHT + stemWidth) / 2 },
        { x: 0, y: (HEIGHT + stemWidth) / 2 },
      ];
